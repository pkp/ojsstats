...
print "Top by countries for 2015: "
filtered_country = filtered[filtered.year==2015].groupby(['country', 'region_id', 'region_name']).archive_id.unique().apply(len).reset_index()
...
print "Regions sorted for 2015: "
filtered_country = filtered[filtered.year==2015].groupby(['region_id', 'region_name']).archive_id.unique().apply(len).reset_index()
...
df = df[(df.year >= 1990) & (df.year<2016)].groupby('year')[0].mean()
print
print df
